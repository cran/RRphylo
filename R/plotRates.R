...
#'@usage plotRates(RR,node,export.tiff =FALSE,filename=NULL)
...
#'@importFrom graphics abline barplot legend hist par
...
plotRates<-function(RR, node, export.tiff=FALSE,filename=NULL){
  #require(phytools)
  ...
  if(export.tiff==TRUE)
  {
    ...
    hist(log(abs(FULLrates[-match(names(shift.rates),rownames(FULLrates)),])),plot=FALSE)->H2
